...
int SerialPort::printf(const char * format, ...)
{
    ...
    int ret = vsnprintf(buf, 300, format, vArgList);
    va_end( vArgList );
...
bool SerialPort::available( long timeOut )
{
        fd_set fds;  
        timeval timeout ;
        timeout.tv_sec = 0;
        ...
        FD_SET(_tty_fd,&fds); //添加描述符  
        int maxfdp=_tty_fd + 1;    //描述符最大值加1  
        
        int ret = select(maxfdp,&fds, NULL, NULL, &timeout);
        if( ret == -1 ) //error
        {
        ...
        {
                 // if(FD_ISSET(_tty_fd,&fds)) //测试  是否可读
            return true;
        }
}
